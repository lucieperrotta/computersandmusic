...
def smr_bit_allocation(params, smr):
    ...
    bits_available = (params.nslots + params.padbit) * SLOT_SIZE \
        - (bits_header + bits_alloc)
...
def model1(samples, params, sfindices):
  ...
  # Scaling so that the maximum magnitude is 96 dB.
    ...
    for sb in range(N_SUBBANDS):
        subband_spl[sb] = np.max(X[int(1 + sb * SUB_SIZE):int(1 + sb
                                 * SUB_SIZE + SUB_SIZE)])
        subband_spl[sb] = np.maximum(subband_spl[sb], 20
                * np.log10(scf[0, sb] * 32768) - 10)
    ...
    for i in range(table.cbnum - 1):
        ...
        for j in range(table.cbound[i], table.cbound[i + 1]):
            if tonal.flag[i] == UNSET:
                ...
                weight += np.power(10, tonal.spl[j] / 10) \
                    * (table.bark[table.map[j]] - i)
        if msum > DBMIN:
            index = weight / np.power(10, msum / 10.0)
            center = table.cbound[i] + np.int(index * (table.cbound[i
                    + 1] - table.cbound[i]))
  ...
  # decimation of tonal components closer than 0.5 Bark
    for i in range(len(tonal.tonecomps) - 1):
        ...
        if table.bark[table.map[this]] - table.bark[table.map[next]] \
            < 0.5:
  ...
  # global masking thresholds
    masking_global = []
    for i in range(table.subsize):
        maskers = (table.hear[i], ) + masking_tonal[i] \
            + masking_noise[i]
