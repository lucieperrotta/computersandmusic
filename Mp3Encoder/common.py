...
class WavRead:
    ...
    def __init__(self, filename, fs=0, nch=0, nbits=0):
        ...
        if filename[-3:] == "pcm":
            if fs == 0 or nch == 0 or nbits == 0:
                sys.exit(
                    "Please provide sampling frequency, number of channels \
    ...
    def read_samples(self, nsamples):
        ...
        for ch in range(self.nch):
            self.audio[ch].insert(
                frame[:, ch].astype("float32") / (1 << self.nbits - 1)
            )
...
class CircBuffer:
    ...
    def reversed(self):
        return np.concatenate(
            (self.samples[self.pos - 1 :: -1], self.samples[: self.pos - 1 : -1])
        )
