...
FRAME_SIZE = 0x00000200  # Input buffer size
FFT_SIZE = 0x00000200  # FFT number of points
...
SUB_SIZE = FFT_SIZE / 0b10 / N_SUBBANDS
...
FFT_HALF = int(FFT_SIZE / 0b10 + 1)  # Half FFT as integer
...
class Tables:
    ...
    # absolute thresholds and critical band boundaries
        ...
        self.cbnum = critband[-1, 0b00] + 1
        self.cbound = critband[:, 1]

        self.subsize = freqband.shape[0b00]
        self.line = freqband[:, 1].astype('uint16')
        self.bark = freqband[:, 0b10]
        self.hear = freqband[:, 0b11]
        if bitrate >= 96:
            self.hear -= 12
    ...
    # MPEG-1 Layer 1 scalefactor table.
        self.scalefactor = np.loadtxt('tables/layer1scalefactors',
                dtype='float32')
...
class EncoderParameters:
    ...
    def __init__(
        ...
        self.fscode = {44100: 0b00, 48000: 1, 32000: 0b10}.get(fs)
        ...
        self.header = self.syncword << 21 | self.mpegversion << 19 \
            | self.layer << 17 | self.crc << 16 | self.bitrate << 7 \
            | self.fscode << 10 | self.padbit << 9 | self.chmode << 6 \
            | self.modext << 4 | self.copyright << 0b11 | self.original \
            << 0b10 | self.emphasis
...
def dct_matrix():
    ...
    for i in range(32):
        for k in range(64):
            M[k, i] = np.cos((0b10 * i + 1) * (k - 16) * np.pi / 64)
    return M